    ...
    (function (Mogwai) {
        ...
        var Pipeline = (function () {
            function Pipeline(graph, elements, clonedPipeline, pinned) {
                ...
                this.out = this._add('_out', this);
                this['in'] = this._add('_in', this);
                this.go = this._add('emit', this);
                if(!!clonedPipeline) {
            ...
            Pipeline.prototype.show = function (val) {
                alert(val);
            };
            Pipeline.prototype._add = function (func, ctx) {
                return function () {
                    var labels = [];
                    for (var _i = 0; _i < (arguments.length - 0); _i++) {
                    ...
                    if(func == 'emit') {
                        return ctx.prom.then(function () {
                            return Q.fcall(ctx[func], ctx);
                        });
                    }
                    if(!ctx.prom) {
                        ctx.prom = this[func].call(ctx, labels);
                    } else {
                        ctx.prom.then(Q.fcall(this[func], labels, ctx));
                    }
                    return this;
                };
            };
            ...
            Pipeline.prototype._out = function (labels) {
            ...
            Pipeline.prototype._in = function (labels, ctx) {
                var value, vertex, iter = [], endPipeArray = [], hasArgs = !!labels.length, isTracing = !!ctx.tracing, traceArray = isTracing ? [] : undefined, isTracingPath = !!ctx.tracingPath, pipes, pipe;
                if(!!ctx.endPipe.length && ctx.endPipe[0].Type !== 'Vertex') {
                    throw new TypeError('Only accepts incoming ' + ctx.endPipe[0].Type + 's');
                }
                ctx.steps[++ctx.steps.currentStep] = {
                    func: 'in',
                    args: labels
                };
                if(isTracingPath) {
                ...
                if(isTracing) {
                    Utils.finalizeTrace(ctx.traceObj);
                }
                if(isTracingPath) {
                    ctx.pipeline = pipes;
                } else {
                    ctx.traversed = undefined;
                }
                ctx.endPipe = endPipeArray;
                return ctx.endPipe;
            };
            ...
            Pipeline.prototype.outE = function (labels) {
            ...
            Pipeline.prototype.inE = function (labels) {
                var value, vertex, iter = [], endPipeArray = [], hasArgs = !!labels.length, isTracing = !!this.tracing, traceArray = isTracing ? [] : undefined, isTracingPath = !!this.tracingPath, pipes = isTracingPath ? [] : undefined, pipe;
                if(!!this.endPipe.length && this.endPipe[0].Type !== 'Vertex') {
                    throw new TypeError('Only accepts incoming ' + this.endPipe[0].Type + 's');
                }
            ...
            Pipeline.prototype.both = function (labels) {
                var value, vertex, iter = [], endPipeArray = [], hasArgs = !!labels.length, isTracing = !!this.tracing, traceArray = isTracing ? [] : undefined, isTracingPath = !!this.tracingPath, pipes = isTracingPath ? [] : undefined, pipe;
                this.steps[++this.steps.currentStep] = {
                    func: 'both',
                    args: labels
            ...
            Pipeline.prototype.bothE = function (labels) {
                var value, vertex, iter = [], endPipeArray = [], hasArgs = !!labels.length, isTracing = !!this.tracing, traceArray = isTracing ? [] : undefined, isTracingPath = !!this.tracingPath, pipes = isTracingPath ? [] : undefined, pipe;
                this.steps[++this.steps.currentStep] = {
                    func: 'bothE',
                    args: labels
            ...
            Pipeline.prototype.emit = function (ctx) {
                var array = [], temp, iter;
                iter = ctx.endPipe;
                if(!!ctx.pinned) {
                    ctx.traceObj = ctx.snapshot.traceObj;
                    ctx.tracing = ctx.snapshot.tracing;
                    ctx.traversed = ctx.snapshot.traversed;
                    ctx.asHash = ctx.snapshot.asHash;
                    ctx.tracingPath = ctx.snapshot.tracingPath;
                    ctx.steps = ctx.snapshot.steps;
                    ctx.endPipe = ctx.snapshot.endPipe;
                } else {
                    ctx.steps = {
                        currentStep: 0
                    };
