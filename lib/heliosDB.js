...
(function (Helios) {
    ...
    (function (Mogwai) {
        ...
        var Compare = (function () {
            ...
            Compare.$eq = function $eq(objVal, val) {
                ...
                while(index) {
                    --index;
                    if(((Utils.isDate(val) && Utils.isDate(objVal[index])) || (Utils.isMoney(val) && Utils.isMoney(objVal[index])) || (!(Utils.isDate(objVal[index]) || Utils.isMoney(objVal[index])))) && (Utils.parseNumber(objVal[index]) === Utils.parseNumber(val))) {
                        return true;
                    }
                }
                return false;
            ...
            Compare.$typeOf = function $typeOf(objVal, val) {
            ...
            Compare.$notTypeOf = function $notTypeOf(objVal, val) {
            ...
            Compare.$in = function $in(objVal, val) {
                ...
                while(index) {
                    ...
                    while(!!i) {
                        --i;
                        if(((Utils.isDate(val[i]) && Utils.isDate(objVal[index])) || (Utils.isMoney(val[i]) && Utils.isMoney(objVal[index])) || (!(Utils.isDate(objVal[index]) || Utils.isMoney(objVal[index])))) && (Utils.parseNumber(objVal[index]) === Utils.parseNumber(val[i]))) {
                            return true;
                        }
                    }
                }
            ...
            Compare.$nin = function $nin(objVal, val) {
            ...
            Compare.$none = function $none(objVal, val) {
                return !Compare.$all(objVal, val);
            };
            Compare.$exact = function $exact(objVal, val) {
                var matches = 0, index = 0, i = 0, valLen = 0;
                val = Utils.unique(val);
                objVal = Utils.unique(objVal);
                valLen = val.length;
            ...
            Compare.$hasAll = function $hasAll(obj, val) {
                var i = val.length, matches = 0, tempObj, tempProp;
                while(!!i) {
                    --i;
                    tempObj = obj;
                    tempProp = val[i];
    ...
    var Utils = (function () {
        ...
        Utils.validISO8601Date = /^([\+-]?\d{4}(?!\d{2}\b))((-?)((0[1-9]|1[0-2])(\3([12]\d|0[1-9]|3[01]))?|W([0-4]\d|5[0-2])(-?[1-7])?|(00[1-9]|0[1-9]\d|[12]\d{2}|3([0-5]\d|6[1-6])))([T\s]((([01]\d|2[0-3])((:?)[0-5]\d)?|24\:?00)([\.,]\d+(?!:))?)?(\17[0-5]\d([\.,]\d+)?)?([zZ]|([\+-])([01]\d|2[0-3]):?([0-5]\d)?)?)?)?$/;
        ...
        Utils.isDate = function isDate(o) {
            var dateResult = Utils.validISO8601Date.exec(o);
        ...
        Utils.parseNumber = function parseNumber(val) {
            ...
            numResult = Utils.validNumeric.exec(val);
            if(numResult != null) {
